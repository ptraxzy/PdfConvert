...
import { ArrowLeft, FileImage, Download, X, Upload, Loader2 } from 'lucide-react';
...
export default function PdfToJpgPage() {
    ...
    const [imageResults, setImageResults] = useState<Blob[]>([]);
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Gagal konversi PDF');
        } finally {
            setIsProcessing(false);
        }
    };

    const handleDownloadImage = (blob: Blob, index: number) => {
    ...
    const handleDownloadAll = () => {
        imageResults.forEach((blob, index) => {
            handleDownloadImage(blob, index);
        });
    };
            ...
            <div className="max-w-4xl mx-auto px-4 pt-24 pb-16">
                ...
                <div className="text-center mb-12 opacity-0 animate-[slideUp_0.6s_ease-out_forwards]">
                    ...
                    <p className="text-xl text-gray-600">
                        Extract halaman PDF jadi gambar JPG berkualitas tinggi
                    </p>
                </div>
                        ...
                        <div className="border-2 border-dashed border-red-200 rounded-2xl p-12 text-center bg-gradient-to-br from-red-50/50 to-transparent hover:border-red-300 transition-colors cursor-pointer">
                            ...
                            <label htmlFor="file-upload" className="cursor-pointer">
                                ...
                                <p className="text-gray-600 mb-4">
                                    Drag & drop atau klik untuk upload file PDF kamu
                                </p>
                    ...
                    <div className="opacity-0 animate-[fadeIn_0.4s_ease-out_forwards]">
                        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 mb-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-gray-600">
                                        {imageResults.length} halaman ready untuk download
                                    </p>
                                </div>
                                <button
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {imageResults.map((blob, index) => (
                                    <div
                                        key={index}
                                        className="group relative bg-gray-50 rounded-xl overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow"
                                    >
                                        <img
                                        ...
                                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                                            <button
                                                onClick={() => handleDownloadImage(blob, index)}
                                                className="opacity-0 group-hover:opacity-100 transition-opacity px-4 py-2 bg-white text-gray-900 rounded-lg font-medium flex items-center gap-2"
                                            >
